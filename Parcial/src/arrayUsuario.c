		...
		if(eUsuario_Vacio(list, len))
		{
			...
			for (i = 0; i < len; i++)
			{
				if(list[i].isEmpty==OCUPADO)
				{
					...
					if(strcmp(list[i].correo,aux.correo)==0 && flagCorreo ==0)
					{
						flagCorreo=1;

					}
					if(strcmp(list[i].password,aux.password)==0 && flagPassword ==0)
					{
						flagPassword=1;

					}
					if(flagCorreo==1 && flagPassword == 1)
					{
						retorno=1;
						break;
					}
...
void eUsuario_MostrarUno(Usuario list){
	...
	printf("|%-15d||%-15d||%-15s||%-15d||%-15s||%-15d||%-15s|\n"
			,list.idUsuario,list.codigoPostal,list.correo,list.isEmpty,list.password,list.tipo,list.direccion);
